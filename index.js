...
const useState = (initialState) => {
  ...
  const setter = (toSetValue) => {
    if (typeof toSetValue === 'function') {
      stateStack[initialState] = toSetValue(stateStack[initialState] ?? initialState)
    } else {
      stateStack[initialState] = toSetValue
    }
  }

  return [stateStack[initialState] ?? initialState, setter]
  // [MODIFICATION ALLOWED]
}
...
const assertArray = (original, asserted) => {
  console.log('RESULT :::::', original, asserted)
